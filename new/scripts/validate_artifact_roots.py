...
def _load_policy(*, root: Path = ROOT) -> dict[str, Any]:
    config_path = root / "new" / "config" / "risk_config.yaml"
    with config_path.open("r", encoding="utf-8") as fh:
        data = yaml.safe_load(fh) or {}
    policy = data.get("artifact_root_policy") or {}
...
def check_production_registry_state(
    ...
    *,
    ...
    state: dict[str, Any] = {
        "path": _repo_relative(registry_path, root=root),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--root",
        default=str(ROOT),
        help="Workspace or extracted review package root to validate.",
    )
    ...
    report = build_report(
        root=root,
        allow_production_active=bool(args.allow_production_active),
    )
    if bool(args.write_report):
        ...
        report_path = (
            ...
            / f"artifact_root_policy_{ts}.json"
        )
        _write_json(report_path, report)
        report["report_path"] = str(report_path)
        report["report_path_relative"] = _repo_relative(report_path, root=root)
