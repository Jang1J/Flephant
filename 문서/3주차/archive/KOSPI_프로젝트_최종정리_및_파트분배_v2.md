...
코어 설계 논문은 총 **6편**이다.
여기에 KOSPI 현지화 보강용 **FAILAB 논문 3편**을 1차 확장 후보로 덧붙인다.
...
## 7-1. FAILAB 논문 매핑 (KOSPI 현지화 보강)

| FAILAB 논문 | 들어가는 위치 | 시점 | 주 담당 |
|---|---|---|---|
| **Momentum HMM (2020)** | Strategy - Market Analyst 보조 신호 (`HMM regime flag`) | **1차 확장** | **AI #1** |
| **Robo-Advisor Instability Index + GA (2020)** | Risk Agent - exposure / cash gating (`InstabilityIndexGate`) | **1차 확장** | **AI #1** |
| **KOSPI200 Prediction Through Low-pass Filtered LSTM (2020)** | Data Agent 전처리 / Quant Prefilter 입력 (`LowPassDenoiser`) | **1차 확장** | **AI #1** |

핵심 해석:
- 위 3편은 기존 6편 구조를 대체하지 않는다.
- 역할은 **KOSPI 현지화 보강 레이어**다.
- 즉, 이번 학기 코어는 여전히 `4-agent runtime loop + 6편 논문 레이어`이고, FAILAB 논문은 **한국시장용 정량/리스크 priors**를 추가하는 방향으로만 반영한다.
...
| 팀원 | 역할 | 비중 |
|---|---|---:|
| **AI #1** | Data Agent + Quant Prefilter + Backtest 계산 엔진<br>+ FAILAB 확장 모듈 (HMM regime detection, Instability Index risk gating, Low-pass denoising) | **30%** |
...
| Low-pass denoising / signal preprocessing | **AI #1** | AI #2 |
| HMM regime detection / market state flag | **AI #1** | AI #2 |
...
| Instability Index risk gating | **AI #1** | AI #2 |
...
2. **코어 6편 논문은 경쟁 관계가 아니라 서로 다른 레이어를 채우는 부품이고, FAILAB 3편은 한국시장 현지화 보강 레이어다.**
...
7. 내부 중간 카드(`QuantEvidenceCard`, `MarketAnalysis`, `TextSignalCard`)의 registry 저장 여부 및 공식 카드와의 관계 정리
